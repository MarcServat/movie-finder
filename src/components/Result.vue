<template>
  <div :class="'results ui container center aligned segment ' + loading">
    <div class="slides fade">
      <div>
        {{ this.search.getPage() }} of {{ this.search.getTotalPages() }} pages
      </div>
      <div class="header">{{ movie.getTitle() }}</div>
      <div class="description">{{ movie.getYear() }}</div>
      <div
        v-if="movie.getPoster() !== 'N/A' && getImage()"
...
export default {
  ...
  methods: {
    ...
    onRight(n) {
      if (
        this.search.getPage() < this.search.getTotalPages() &&
        this.slide.pos === this.slide.max
      ) {
        this.requestImages(n);
      } else {
        this.slide.pos === this.search.getTotalPages()
          ? this.slide.pos
          : (this.slide.pos += n);
        this.movie = this.movieData.data[this.slide.pos];
      }
